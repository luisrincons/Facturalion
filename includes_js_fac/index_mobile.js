...
$(function(){
	if(typeof localStorage === 'undefined'){
		  new PNotify({
			  title: 'Patrón de accesso',
			  text: "No existe un patrón almacenado.",
			  type: 'dark',
			  delay: 1000
		  });
	} else {
	    //Recupera patrón
		var pattern_data = localStorage.getItem("pattern_data");	
		var	pattern_draw = localStorage.getItem("pattern_draw");
		//Valida patrón
		mhnUI.setup(pattern_data, pattern_draw);
	}
});
mhnUI = {
    pattern: "",
    setup: function(data, pad) {
        this.lock(data, pad)
    },
    lock: function(data, pad) {
        mhnUI.page.hide(), 
		pattern = new PatternLock(".mhn-lock", {radius:20, margin: 20}), 
 		$(".mhn-lock-title").html($(".mhn-lock-title").data("title")), pattern.checkForPattern(pad, function() {
			var pattcode = pattern.getPattern();
			//var encrcode = AesCtr.encrypt(data, pattcode, 256);
			var decrcode = AesCtr.decrypt(data, pattcode, 256);
 			GetDataUser(pattcode, decrcode);
		}, function() {
...
function GetDataUser(pattcode, decrcode){
	
   $.ajax({
	 type: 'POST',
	 url: 'http://cdcom.dynalias.com/facturalion2/includes_php_fac/_validate.php',
	 data: {'type_access':'pattern', 'pattern_data':decrcode, 'pattern_draw':pattcode},
	 dataType: 'json',
