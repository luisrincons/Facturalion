...
$(document).ready(function() {
    $('#icons').load('http://cdcom.dynalias.com/facturalion2/includes_php_fac/getdata_invoices_year.php');
	$(document).on("click", ".item_year", function() {
...
function save_selection_item(idgral){
  $.ajax({
    type: 'POST',
    url: 'http://cdcom.dynalias.com/facturalion2/includes_php_fac/saveitemdata_year.php',
	data: {'id':idgral},
    dataType: 'json',
    beforeSend: function(xhr) {
      $.spin('true');      
	...
	if(data.result == "Ok!"){
      window.location ="invoices_month.html";
	} else {
